...
function hintHandler(e) {
  ...
  if (pressedKey == "Enter" || pressedKey == "U+0000") {
...
function execSelect(elem) {
  ...
  if (tag_name == "a" && elem.href != "") {
    ...
    // TODO: ajax, <select>
    if (hint_open_in_new_tab) window.open(elem.href);
...
function setHints() {
  ...
  var elems = document.body.querySelectorAll(
    "a, input:not([type=hidden]), textarea, select, button"
  );
...
function get_key(evt) {
  ...
  if (evt.shiftKey) {
    ...
    if (
      /^(Enter|Space|BackSpace|Tab|Esc|Home|End|Left|Right|Up|Down|PageUp|PageDown|F(\d\d?))$/.test(
        key
      )
    )
